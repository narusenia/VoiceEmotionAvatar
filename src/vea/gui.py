...
class VeaGui:
        ...
        with dpg.window(label="VoiceEmotionAvatar", tag="main_window"):
            ...
            with dpg.group(horizontal=True):
                ...
                self._volume_bar = dpg.add_progress_bar(
                    default_value=0.0, overlay="--", width=-80, tag="volume_bar",
                )
            ...
            with dpg.group(horizontal=True):
                ...
                dpg.add_button(label="Stop", callback=self._on_stop_clicked, tag="stop_btn", enabled=False)
            ...
            with dpg.group(horizontal=True):
                ...
                dpg.add_checkbox(
                    label="Full Mode (7 emotions)",
                    default_value=False,
                    callback=self._on_full_mode_toggle,
                    tag="full_mode_cb",
                )
            ...
            dpg.add_slider_float(
                label="Instant Threshold", default_value=0.4,
                min_value=0.1, max_value=0.9,
                callback=self._on_instant_threshold_slider, width=200,
            )
            dpg.add_slider_float(
                label="Smoothing", default_value=0.5,
                min_value=0.0, max_value=1.0,
                callback=self._on_instant_smoothing_slider, width=200,
            )
            ...
            dpg.add_slider_float(
                label="Lerp Speed", default_value=default_lerp,
                min_value=0.01, max_value=1.0,
                callback=self._on_lerp_slider, width=200,
            )
            dpg.add_slider_float(
                label="Hysteresis", default_value=default_hysteresis,
                min_value=0.0, max_value=0.5,
                callback=self._on_hysteresis_slider, width=200,
            )
            dpg.add_slider_float(
                label="Silence Threshold", default_value=default_silence,
                min_value=0.001, max_value=0.1, format="%.3f",
                callback=self._on_silence_slider, width=200,
            )
            dpg.add_slider_float(
                label="Hold (sec)", default_value=0.0,
                min_value=0.0, max_value=5.0, format="%.1f",
                callback=self._on_hold_slider, width=200,
            )

            dpg.add_separator()
            if dpg.add_collapsing_header(label="Advanced Settings"):
                dpg.add_input_text(label="OSC IP", default_value=default_osc_ip, tag="osc_ip_input", width=200)
                dpg.add_input_int(label="OSC Port", default_value=default_osc_port, tag="osc_port_input", width=200)
    ...
    def _on_full_mode_toggle(self, sender, value, user_data) -> None:
    ...
    def _on_hysteresis_slider(self, s, v, u) -> None:
        if self._on_hysteresis_change: self._on_hysteresis_change(v)

    def _on_silence_slider(self, s, v, u) -> None:
        if self._on_silence_change: self._on_silence_change(v)

    def _on_gain_slider(self, s, v, u) -> None:
        if self._on_gain_change: self._on_gain_change(v)

    def _on_instant_toggle(self, s, v, u) -> None:
        if self._on_instant_mode_change: self._on_instant_mode_change(v)

    def _on_instant_threshold_slider(self, s, v, u) -> None:
        if self._on_instant_threshold_change: self._on_instant_threshold_change(v)

    def _on_instant_smoothing_slider(self, s, v, u) -> None:
        if self._on_instant_smoothing_change: self._on_instant_smoothing_change(v)

    def _on_hold_slider(self, s, v, u) -> None:
        if self._on_hold_change: self._on_hold_change(v)
    ...
    def setup(
        ...
        dpg.create_context()
        dpg.create_viewport(title="VoiceEmotionAvatar", width=500, height=750)
        self._build_ui(
